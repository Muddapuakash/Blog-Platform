...
def admin_get_all_posts():
    ...
    return jsonify(post_service.get_all_posts(page, per_page, status)), 200
...
def admin_get_all_comments():
    ...
    return jsonify(
        comment_service.get_all_comments(page, per_page, status)
    ), 200


# ======================================================
# 🔁 BACKWARD-COMPATIBLE COMMENT HANDLER
# (EDIT / APPROVE / REJECT via SAME ROUTE)
# ======================================================

@admin_bp.route('/comments/<int:comment_id>', methods=['PUT'])
@admin_required
def admin_update_or_moderate_comment(comment_id):
    data = request.get_json()
...
@admin_bp.route('/posts/upload', methods=['POST'])
@admin_required
def upload_image():
